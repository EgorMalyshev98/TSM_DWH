...
FROM (
	...
	digest(TRIM(COALESCE("СтруктураРаботИдентификатор"::varchar, 'N\A')) || '|' || TRIM(COALESCE("СтруктураРаботУникальныйКод"::varchar, 'N\A')) || '|' || TRIM(COALESCE("ОбъемРаботы"::varchar, 'N\A')) || '|' || TRIM(COALESCE("Примечание"::varchar, 'N\A')) || '|' || TRIM(COALESCE("СтруктураРаботНомерКВ"::varchar, 'N\A')) || '|' || TRIM(COALESCE("СтруктураРаботПометкаУдаления"::varchar, 'N\A')) || '|' || TRIM(COALESCE("ВидРаботУровеньОперации"::varchar, 'N\A')) || '|' || TRIM(COALESCE("СтруктураРабот_value"::varchar, 'N\A')) || '|' || TRIM(COALESCE("СтруктураРабот_codе"::varchar, 'N\A')) || '|' || TRIM(COALESCE("СтруктураРабот_name"::varchar, 'N\A')) || '|' || TRIM(COALESCE("ВидРабот_value"::varchar, 'N\A')) || '|' || TRIM(COALESCE("ВидРабот_codе"::varchar, 'N\A')) || '|' || TRIM(COALESCE("ВидРабот_name"::varchar, 'N\A')), 'sha1') as hdiff_dv_sat_fact_work,
